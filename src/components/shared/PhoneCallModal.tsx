...
export default function PhoneCallModal({ isOpen, onClose }: PhoneCallModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-2 sm:p-4 overflow-hidden">
      <div className="bg-gray-900 rounded-2xl sm:rounded-3xl w-full max-w-sm h-[95vh] sm:h-[700px] flex flex-col shadow-2xl text-white relative overflow-hidden">
        {/* Close button */}
        <button 
          onClick={() => {
            onClose();
          ...
          className="absolute top-3 right-3 sm:top-4 sm:right-4 z-10 text-white/70 hover:text-white transition-colors"
        ...
        {/* Status bar */}
        <div className="flex justify-between items-center p-3 sm:p-4 text-xs sm:text-sm">
          <span>9:41</span>
          <div className="flex space-x-1">
            <div className="w-3 h-1.5 sm:w-4 sm:h-2 bg-white rounded-sm"></div>
            <div className="w-3 h-1.5 sm:w-4 sm:h-2 bg-white rounded-sm"></div>
            <div className="w-3 h-1.5 sm:w-4 sm:h-2 bg-white/50 rounded-sm"></div>
          </div>
        </div>
        ...
        {callState === 'incoming' && (
          <>
            ...
            <div className="flex-1 flex flex-col items-center justify-center space-y-6 sm:space-y-8 px-6 sm:px-8">
              <div className="text-center">
                <p className="text-xs sm:text-sm text-gray-400 mb-2">Incoming call</p>
                <h2 className="text-xl sm:text-2xl font-semibold mb-1">Sakura Sushi</h2>
                <p className="text-sm sm:text-base text-gray-400">+1 (555) 123-4567</p>
              </div>
              
              <div className="w-28 h-28 sm:w-32 sm:h-32 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center">
                <Phone className="w-10 h-10 sm:w-12 sm:h-12 text-white" />
              </div>
              
              <div className="text-center">
                <p className="text-xs sm:text-sm text-gray-400">AI Agent Available 24/7</p>
              </div>
            </div>
            ...
            <div className="p-6 sm:p-8">
              <div className="flex justify-center space-x-12 sm:space-x-16">
                <button 
                  onClick={endCall}
                  className="w-14 h-14 sm:w-16 sm:h-16 bg-red-500 rounded-full flex items-center justify-center hover:bg-red-600 active:scale-95 transition-all"
                ...
                <button 
                  onClick={answerCall}
                  className="w-14 h-14 sm:w-16 sm:h-16 bg-green-500 rounded-full flex items-center justify-center hover:bg-green-600 active:scale-95 transition-all"
          ...
          <>
            ...
            <div className="flex-1 flex flex-col items-center justify-center px-4 sm:px-8">
              ...
              <div className="text-center mb-6 sm:mb-8">
                <h2 className="text-lg sm:text-xl font-semibold mb-1">Sakura Sushi</h2>
                <p className="text-green-400 text-xs sm:text-sm mb-2">Connected</p>
                <p className="text-gray-400 text-xs sm:text-sm">{formatTime(callDuration)}</p>
              </div>

              {/* Avatar with speaking animation */}
              <div className="flex justify-center mb-8 sm:mb-12">
                <div className={`w-28 h-28 sm:w-32 sm:h-32 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center transition-all duration-300 ${
              ...
              <div className="flex-1 flex items-center justify-center px-4 mb-6 sm:mb-8 w-full">
                <div className="bg-gray-800 rounded-2xl p-4 sm:p-6 max-w-xs w-full text-center">
                  <div className="flex items-center justify-center space-x-2 mb-2 sm:mb-3">
                    <div className={`w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full ${isAISpeaking ? 'bg-blue-500' : 'bg-gray-500'}`}></div>
              ...
              <div className="px-4 sm:px-8 pb-6 sm:pb-8 w-full">
                <div className="flex justify-center space-x-4 sm:space-x-8">
                  ...
                  <button 
                    className="w-12 h-12 sm:w-14 sm:h-14 bg-gray-700 hover:bg-gray-600 rounded-full flex items-center justify-center transition-all active:scale-95"
                    aria-label="Speaker"
                  >
                  ...
                  <button 
                    onClick={endCall}
                    className="w-12 h-12 sm:w-14 sm:h-14 bg-red-500 hover:bg-red-600 rounded-full flex items-center justify-center transition-all active:scale-95"
                    aria-label="End call"
                  >
        ...
        {callState === 'ended' && (
          <div className="flex-1 flex flex-col items-center justify-center space-y-6 sm:space-y-8">
            <div className="text-center">
              <h2 className="text-lg sm:text-xl font-semibold mb-2">Call Ended</h2>
              <p className="text-sm sm:text-base text-gray-400">Duration: {formatTime(callDuration)}</p>
            </div>
            <div className="w-14 h-14 sm:w-16 sm:h-16 bg-gray-700 rounded-full flex items-center justify-center">
              <PhoneOff className="w-7 h-7 sm:w-8 sm:h-8 text-gray-400" />
            </div>
          </div>
        )}
      </div>
